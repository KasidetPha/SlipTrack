-- ================================================================
-- SlipTrack: Full DB Bootstrap (MySQL / MariaDB compatible)
-- ================================================================

-- 0) Drop & Recreate database
DROP DATABASE IF EXISTS sliptrack;
CREATE DATABASE sliptrack
...
-- ================================================================
-- 1) Drop objects if exist (safety, in FK order)
-- ================================================================
...
-- Income Categories  (Salary, Wages, Gift, Sales)
...
-- Users (hash in SQL using SHA2; production ควรใช้ bcrypt/argon2)
INSERT INTO users
...
-- Receipt Items (ensure totals match receipts)
...
-- Incomes (~20+ rows; only users 3,4,5; include Sep & Oct 2025)
...
-- ================================================================
-- 4) VALIDATION & ANALYTICS QUERIES
-- ================================================================

-- A) ตรวจยอดใบเสร็จ = ผลรวมรายการ (ควรได้ 0 แถว)
SELECT
  r.receipt_id,
  r.total_amount,
...
-- B) จำนวนใบเสร็จต่อร้านต่อเดือน (มี Sep & Oct 2025)
SELECT
  s.store_name,
...
-- C) ไม่มีใบเสร็จผูกกับ Admin (ควรได้ 0 แถว)
...
-- D) ยอดรายรับรวมต่อผู้ใช้ต่อเดือน
SELECT
  u.user_id,
  u.username,
...
-- F) Net (Income - Expense) per user per month  [NO CTE version]
SELECT
  ks.user_id,
  ks.m AS month_key,
  COALESCE(i.income_total, 0)  AS income_total,
  COALESCE(e.expense_total, 0) AS expense_total,
  COALESCE(i.income_total, 0) - COALESCE(e.expense_total, 0) AS net_balance
FROM (
  SELECT user_id, DATE_FORMAT(receipt_date, '%Y-%m') AS m
  FROM receipts
  UNION
  SELECT user_id, DATE_FORMAT(income_date, '%Y-%m') AS m
  FROM incomes
) AS ks
LEFT JOIN (
  SELECT user_id,
         DATE_FORMAT(income_date, '%Y-%m') AS m,
         SUM(amount) AS income_total
  FROM incomes
  GROUP BY user_id, DATE_FORMAT(income_date, '%Y-%m')
) AS i
  ON i.user_id = ks.user_id AND i.m = ks.m
LEFT JOIN (
  SELECT user_id,
         DATE_FORMAT(receipt_date, '%Y-%m') AS m,
         SUM(total_amount) AS expense_total
  FROM receipts
  GROUP BY user_id, DATE_FORMAT(receipt_date, '%Y-%m')
) AS e
  ON e.user_id = ks.user_id AND e.m = ks.m
ORDER BY ks.user_id, ks.m;
